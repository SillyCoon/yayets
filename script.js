...
async function generateRoute() {
	...
	try {
		...
		const response = await fetch(
			`${ORS_API_URL}?api_key=${CONFIG.ORS_API_KEY}&start=${startPoint.lng},${startPoint.lat}&end=${endPoint.lng},${endPoint.lat}`,
		);
		...
		routeCoordinates = data.features[0].geometry.coordinates.map((coord) => [
			coord[1],
			coord[0],
		]);
...
function startTracking() {
	...
	trackingInterval = navigator.geolocation.watchPosition(
		(position) => {
			...
			document.getElementById("currentTime").textContent = new Date(elapsed)
				.toISOString()
				.substr(11, 8);
		},
		(error) => {
...
function handleOrientation(event) {
	...
	if (heading !== undefined && !Number.isNaN(heading)) {
		...
		const smoothedHeading =
			headingBuffer.reduce((a, b) => a + b, 0) / headingBuffer.length;
		const previousHeading = userHeading;
		// Only update if change is significant
		if (
			Math.abs(smoothedHeading - previousHeading) > HEADING_UPDATE_THRESHOLD
		) {
...
window.addEventListener("directionChanged", (e) => {
	...
	console.log(
		`Direction changed by ${Math.round(change)}° to ${Math.round(heading)}°`,
	);
